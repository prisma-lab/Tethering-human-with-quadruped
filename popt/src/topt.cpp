...
void get_trajectory( Eigen::VectorXd com_p, Eigen::VectorXd com_dp, Eigen::VectorXd dcom_p, Eigen::Vector3d eeBLpos, Eigen::Vector3d eeBRpos, 
    Eigen::Vector3d eeFLpos, Eigen::Vector3d eeFRpos, int gait_flag, float duration, towr::SplineHolder & solution, towr::NlpFormulation & formulation_, const double Ixx_value, const double Iyy_value, const double Izz_value, const double Ixy_value, const double Ixz_value, const double Iyz_value ) {


    formulation_.terrain_ = std::make_shared<towr::FlatGround>(0.0);
    formulation_.model_ = towr::RobotModel(towr::RobotModel::Dogbot, Ixx_value, Iyy_value, Izz_value, Ixy_value, Ixz_value, Iyz_value);
    ...
    std::cout<<"posinit "<< formulation_.initial_base_.lin.at(towr::kPos)<<std::endl;
    std::cout<<"posinit "<< formulation_.initial_base_.ang.at(towr::kPos)<<std::endl;
    ...
    formulation_.final_base_.ang.at(towr::kPos) << 0.0, 0.0, dcom_p(5);
     ...
     std::cout<<"posinit "<< formulation_.final_base_.lin.at(towr::kPos)<<std::endl;
     std::cout<<"posinit "<< formulation_.final_base_.ang.at(towr::kPos)<<std::endl;
