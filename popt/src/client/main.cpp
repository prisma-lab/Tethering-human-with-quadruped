...
DOGCTRL::DOGCTRL() {
    ...
    estimation_pub = _nh.advertise<geometry_msgs::WrenchStamped>("estimation_ee", 1000);
    traj_des_pub = _nh.advertise<geometry_msgs::PointStamped>("traj_des", 1000);
    traj_pub = _nh.advertise<geometry_msgs::PointStamped>("traj", 1000);
...
{   
    ...
    //cout<<"world update"<<worldeigen<<endl;
   ...
   /* for (int i=0; i<kinDynComp.getNrOfFrames(); i++)
    {
     ...
     //cout<<"link info"<<kinDynComp.getFrameName(i)<<endl;
...
Eigen::Matrix<double,6,6> m=(Eigen::Matrix<double,6,6>::Identity()+k[0]*T).inverse()*k[0];
  ...
  cout<<"wext"<<w[0]<<endl;
  cout<<"mass"<<rho-yw_prev[0]-yd[0]<<endl;
    cout<<"mass"<<m<<endl;
...
void DOGCTRL::ctrl_loop() {
    ...
    CoMPosDes << toEigen(CoM)[0]+0.05*sin(toEigen(CoM)[5]), toEigen(CoM)[1]-0.05*cos(toEigen(CoM)[5]), 0.39, 0,  0, toEigen(CoM)[5];
  ...
  traj_pub.publish( p );
    ...
    //if(abs(angles::shortest_angular_distance(  toEigen(CoM)[5],  pos_cmd_ang))>=0.3)
    //{CoMPosDes << toEigen(CoM)[0], toEigen(CoM)[1]-0.05-pos_adm, 0.39, 0,  0, 0;}
    //else
   // //{
   // if(toEigen(CoM)[5]>2)
   // {
  ...
  // CoMPosDes << toEigen(CoM)[0], toEigen(CoM)[1]-(0.05+pos_adm), 0.39, 0,  0,  0;
        ...
        if(abs(w[0][0])<abs(w[0][1]))
     {
           CoMPosDes << toEigen(CoM)[0]+(0.05+pos_adm)*sin(toEigen(CoM)[5]), toEigen(CoM)[1]-(0.05+pos_adm)*cos(toEigen(CoM)[5]), 0.39, 0,  0,  toEigen(CoM)[5];
       }
       else if(abs(w[0][0])>abs(w[0][1]) && toEigen(CoM)[5]<1.5){
           CoMPosDes << toEigen(CoM)[0], toEigen(CoM)[1], 0.39, 0,  0,  toEigen(CoM)[5]+0.07;
       }
       else
       {
           CoMPosDes << toEigen(CoM)[0]+(0.05+pos_adm)*sin(toEigen(CoM)[5]), toEigen(CoM)[1]-(0.05+pos_adm)*cos(toEigen(CoM)[5]), 0.39, 0,  0,  toEigen(CoM)[5];
   ...
   // }
    ...
    //  }
    //  else{
    //   CoMPosDes << toEigen(CoM)[0]+(0.05+pos_adm)*sin(toEigen(CoM)[5]+0.07), toEigen(CoM)[1]-(0.05+pos_adm)*cos(toEigen(CoM)[5]+0.07), 0.39, 0,  0, toEigen(CoM)[5]+0.07;
    //  }
    //}

    cout<<"Ang desired"<<pos_adm<<endl;
  ...
  /*  while((ros::Time::now()-begin).toSec() <   formulation.params_.ee_phase_durations_.at(0)[0] ) {
      ...
      CoMPosDes << toEigen(CoM)[0], toEigen(CoM)[1]-0.05, 0.39, 0,  0, toEigen(CoM)[5];
